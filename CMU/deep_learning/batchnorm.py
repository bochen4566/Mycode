...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        if eval == False:
            # training mode
            self.NZ = (Z - self.O.dot(self.M)) / (self.O.dot(np.sqrt(self.V + self.eps)))  # TODO
            
            self.BZ = self.NZ*(self.O.dot(self.BW)) + self.O.dot(self.Bb)# TODO
            ...
            # inference mode
            self.NZ = (Z - self.O.dot(self.running_M)) / (self.O.dot(np.sqrt(self.running_V + self.eps)))  # TODO
            self.BZ = self.NZ*self.O.dot(self.BW) + self.O.dot(self.Bb)  # TODO
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -0.5*np.sum(dLdNZ*(self.Z-self.M)*np.power(self.V+self.eps, -3/2), axis=0, keepdims=True)  # TODO
        dZdM = -np.power(self.V+self.eps, -1/2) - 0.5*(self.Z-self.M)*np.power(self.V+self.eps, -3/2)*(- (2/self.N)*np.sum(self.Z - self.M, axis=0, keepdims=True))
        dLdM = np.sum(dLdNZ*dZdM, axis=0, keepdims=True)  # TODO

        dLdZ = dLdNZ*np.power(self.V+self.eps, -1/2) + dLdV*((2/self.N)*(self.Z - self.M)) + dLdM/self.N # TODO
